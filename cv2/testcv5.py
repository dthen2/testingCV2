...
def make_conv_org(blur_dist):
    ...
    gain = 1./256.  #3->1, 2.5->0.3, 3.5->0.2
    dist_limit = 3.
    for x in range(size):
        for y in range(size):
...
if __name__ == '__main__':
    ...
    if image == 'normal_cam':
        ...
        blurred_rate, filen = 2.0,'IMG_5527.jpg' # ナナとワーム。床のマットの輪郭がくっきり
        img = cv2.imread(filen) 
        sh =2
        blur_dist = 3.  # この値を4以上にするとメモリが不足する。ピクセル数がディスクリートのため、この値を少し変えるとおかしくなる。
    else:
