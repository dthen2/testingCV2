...
class PointCloud:
    ...
    def cloud_division(self):
        ...
        return(new_clouds)   # 分割した点群のリストを返す。分割が無ければ空リスト
...
class Closest:
    ...
    def closest_point_diff(self, resolution, rot_scale, x_scale, mag_scale):
        rot_prev = self.rotation
        sift_prev = self.sift
        mag_prev = self.magnify
        diff_rot = []
        diff_x = []
        diff_mag = []
...
cloud = PointCloud(2, points=refpoints)
...
def main2():
    ...
    new_clouds = cloud.cloud_division() # 現在不調・・・正しいが、その前の points_line_sort で壊してる
